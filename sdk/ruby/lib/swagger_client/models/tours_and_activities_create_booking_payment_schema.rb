...
module SwaggerClient
  class ToursAndActivitiesCreateBookingPaymentSchema
    # Credit card number.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ToursAndActivitiesCreateBookingPaymentSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ToursAndActivitiesCreateBookingPaymentSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'card_cvv')
        self.card_cvv = attributes[:'card_cvv']
      end
      ...
      if attributes.key?(:'billing_zip')
        self.billing_zip = attributes[:'billing_zip']
      end
    ...
    def hash
      [card_number, card_expiration_month, card_expiration_year, card_cvv, card_name, billing_address, billing_city, billing_state, billing_zip, billing_country].hash
    end
