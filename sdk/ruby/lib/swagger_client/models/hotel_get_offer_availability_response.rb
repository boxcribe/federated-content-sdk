...
module SwaggerClient
  class HotelGetOfferAvailabilityResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::HotelGetOfferAvailabilityResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::HotelGetOfferAvailabilityResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [offer_id, supplier_offer_id, offer_index, offer_tax, offer_price, offer_currency, offer_cancelation_policies, checkin_date, checkout_date, hotel_name, hotel_description, hotel_address, hotel_latitude, hotel_longitude, hotel_distance, hotel_distance_metric, hotel_images, hotel_rating, hotel_reviews, hotel_stars, hotel_checkin_time, hotel_checkout_time, hotel_facilities, hotel_amenities, hotel_policies, rooms].hash
    end
