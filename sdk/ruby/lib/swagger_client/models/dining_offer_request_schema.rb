...
module SwaggerClient
  class DiningOfferRequestSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DiningOfferRequestSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DiningOfferRequestSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [location_latitude, location_longitude, location_radius, date, filter_by_keyword, filter_by_price_min, filter_by_price_max, filter_by_distance_min, filter_by_distance_max, filter_by_rating_min, filter_by_rating_max, filter_by_subcategories, sort_by, sort_order, page, limit, language, currency, metric].hash
    end
